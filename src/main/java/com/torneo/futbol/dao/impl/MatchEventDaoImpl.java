...
public class MatchEventDaoImpl implements MatchEventDao{
    ...
    public List<MatchEvent> getMatchEventsByMatch(Long matchId){
        Optional<MatchEvent> eventOptional = matchEventRepository.findById(matchId);
        return eventOptional.map(Collections::singletonList).orElse(Collections.emptyList());
    }
